...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrr   , I, word_t t = CSRs(imm); CSRs(imm) = t | src1; R(rd) = t);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = isa_raise_intr(R(17) , s->snpc-4));  // R(17) : a7
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrw   , I, word_t t = CSRs(imm); CSRs(imm) = src1; R(rd) = t);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (int)src1 >> (src2 & 0x3f));

  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if (src1 < src2) s->dnpc = s->pc + imm);

  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src1 / src2);  
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = src1 % src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((int64_t)(int)src1 * (int64_t)(int)src2) >> 32);
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int)src1 >> (imm & 0x3f));
